...
const Project2 = () => {
  return (
    <div className="project1">
      ...
      To study the trends of car insurance fraud, 5 different predictive models were generated with my particular focus being on LASSO Regression which involved hyper-parameter tuning using all car and claim-maker features available. The algorithm focussed on determining a model consisting of only the most statistically significant features required to make high-precision decisions. The model performed sufficiently on the test set with higher precision by 3.9% and 1.4%, and since for an insurance company we would like to maximize True Positives (TP) and minimize False Positives (FP), this augurs well for the model fitment.
...
features contributed to overall data putting the model at a risk of overfitting.
Alternatively, a check was conducted using a very small value of C (0.001), which did not allow
any features to contribute, reducing model accuracy significantly and leading to underfitting.
These two cases, however, proved to be good sanity checks for the regression analysis.
To correctly tune C, the values were iteratively decreased from 10 till 5 (step down = 1), where
the first feature “Medium Vehicle” was dropped. The next feature to be dropped was “Vehicle
Color Other” (C=2.0). The step down was reduced to 0.5 and further to 0.1 to check for
parameter sensitivity. A complete drop guide of features and their accuracies is shown below.
     </div> 
    <div style={{backgroundColor:"black",color:"white"}} >
      ...
      Additionally detailed analysis of all features showed that “Age of Driver, Safety Rating, High
      Education, Zip Code, Past no. of Claims, Age of Vehicle, Vehicle Price, Large Vehicles,
      Black Vehicles and Marital Status” were amongst the most statistically significant to the
      model and had higher predictive power. 
...
1.4%, and since for an insurance company we would like to maximize True Positives (TP) and
minimize False Positives (FP), this augurs well for the model fitment.
    </div>
